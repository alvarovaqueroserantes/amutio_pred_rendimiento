...
if uploaded_file:
    ...
    if modo == "Resumen global":
        ...
        st.metric("🔹 Mejor semana para cosechar", semana_optima)
        st.metric("🔹 Semanas hasta cosecha", semanas_restantes)
        ...
        if uploaded_forecast:
            ...
            for _, row in df_forecast_p.iterrows():
                ...
                temp_alert = "Sí" if row["temp_media"] > 28 else "No"
                lluvia_alert = "Sí" if row["lluvia"] < 2 else "No"
